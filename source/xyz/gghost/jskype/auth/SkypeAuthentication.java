package xyz.gghost.jskype.auth;
...
public class SkypeAuthentication {
    ...
    public void authLogin(SkypeAPI api, Skype account) throws FailedToLoginException{
        ...
        account.setRegToken(packet.getCon().getHeaderField("Set-RegistrationToken").split(";")[0]);
        account.setEndPoint(packet.getCon().getHeaderField("Set-RegistrationToken").split(";")[2].split("=")[1]);
    }
    ...
    public boolean save(SkypeAPI api, Skype account) {
        ...
        packet.setData(id);
        packet.setUrl("https://" + url + "/v1/users/ME/endpoints/" + account.getEndPoint() + "/presenceDocs/messagingService");
